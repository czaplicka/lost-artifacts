...
export class ShoeprintScene extends BaseScene {
  ...
  showEntryButtons() {
    ...
    // Jeden przycisk na jeden wpis z bazy, tekst rozbity na 2 linie,
    // żeby się mieścił na ekranie (Phaser Text respektuje '\n').
    this.entries.forEach((entry) => {
      const line1 = `${entry.id}  |  SIZE ${entry.size}`;
      const line2 = `${entry.sole}  |  ${entry.wear}`;
      const label = `${line1}\n${line2}`;
