...
export class MonologueManager {
    ...
    handleResize(gameSize) {
        ...
        this.border.setPosition(o.panelMarginX + 5, panelY + 5);
        this.border.setSize(panelWidth - 10, o.panelHeight - 10);
    ...
    say(text, options = {}) {
        ...
        if (
            !settings.force
            && this.isOnCooldown(settings.cooldownKey, settings.cooldownMs)
        ) {
            return false;
        }
    ...
    sayRandom(key, options = {}) {
        ...
        if (!lines.length) {
            console.warn(`[MonologueManager] No lines found for key: "${key}"`);
            return false;
        }
    ...
    getRandomLine(key, lines) {
        ...
        const availableLines = lines.filter((line) => line !== previousLine);
    ...
    isOnCooldown(key, cooldownMs) {
        ...
        return (this.scene.time.now - lastPlayedAt) < cooldownMs;
    }
