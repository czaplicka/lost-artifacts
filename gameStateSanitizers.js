import { createDefaultReconstructedHeist, defaultGameState } from './GameData.js';

// --- Stałe walidacyjne ---
export const VALID_SLOT_FEEDBACK_STATUSES = new Set(['correct', 'wrong-position', 'absent']);
export const VALID_MONEY_SOURCES = new Set(['cash', 'agency']);
export const VALID_MONEY_CATEGORIES = new Set([
  'travel', 'hotel', 'food', 'drink', 'newspaper', 'phone', 'taxi', 
  'informant', 'side_case', 'mission_reward', 'office_upgrade', 'item', 
  'agency_advance', 'refund'
]);
export const VALID_DIFFICULTIES = new Set(['rookie', 'field', 'master']);
export const VALID_CLUE_SCOPES = new Set(['crime_scene', 'route_leg', 'finale']);
...
// --- Funkcje sanitizing dla obiektów ---
...
export function sanitizeReconstructedHeist(data) {
  ...
  return {
    ...
    playerFinalText: typeof safeData.playerFinalText === 'string' ? safeData.playerFinalText : defaults.playerFinalText,
    playerSkills: sanitizeStringArray(safeData.playerSkills),
    playerTheoryScore: Number.isFinite(safeData.playerTheoryScore) ? safeData.playerTheoryScore : defaults.playerTheoryScore,
    playerTheoryResult: (typeof safeData.playerTheoryResult === 'string' || safeData.playerTheoryResult === null) ? safeData.playerTheoryResult : defaults.playerTheoryResult,
    playerSlotFeedback: sanitizeSlotFeedbackArray(safeData.playerSlotFeedback),
    playerAttemptsLeft: Number.isInteger(safeData.playerAttemptsLeft) ? Phaser.Math.Clamp(safeData.playerAttemptsLeft, 0, 4) : defaults.playerAttemptsLeft
  };
}
...
export function sanitizeSaveData(data) {
